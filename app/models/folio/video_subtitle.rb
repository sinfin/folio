...
class Folio::VideoSubtitle < Folio::ApplicationRecord
  ...
  def mark_transcription_ready!(text_content)
    ...
    if valid?
      ...
      # Save invalid subtitles but keep disabled
      self.enabled = false
      update_transcription_metadata(
        state: 'ready',
        completed_at: Time.current.iso8601,
  ...
  def transcription_job_class
    transcription&.dig('job_class')&.constantize if auto_generated?
  end
  ...
  def last_validated_valid?
    validation&.dig('is_valid') || false
  end
  ...
  def status_message
    case status_for_display
    when 'processing'
      I18n.t('folio.video_subtitle.status.processing')
    when 'ready'
      I18n.t('folio.video_subtitle.status.ready')
    when 'disabled'
      I18n.t('folio.video_subtitle.status.disabled')
    else
      I18n.t('folio.video_subtitle.status.empty')
    end
  end
  ...
  def update_manual_edits_metadata
    current_edits = manual_edits || {}
    self.manual_edits = current_edits.merge(
      'last_edited_at' => Time.current.iso8601,
      'edit_count' => (current_edits['edit_count'] || 0) + 1
    )
  end
  ...
  def validate_vtt_format
    ...
    lines.each_with_index do |line, index|
      ...
      unless last_line_was_timecode
        errors.add(:text, :invalid_subtitle_block, line: index + 1)
        return
      end
    end
  end
