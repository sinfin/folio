...
module Folio::FriendlyId::UniqueSlugForMultipleClasses
  ...
  private
    def slug_already_exists?(slug)
      slugs = FriendlyId::Slug
      if defined?(self.class::FRIENDLY_ID_SCOPE)
        slugs = slugs.where(scope: "#{self.class::FRIENDLY_ID_SCOPE}:#{send(self.class::FRIENDLY_ID_SCOPE)}")
      end
      slugs.where(sluggable_type: self.class.slug_classes.map { |x| x.to_s }, slug:).exists?
    end
