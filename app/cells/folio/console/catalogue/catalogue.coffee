$ ->
  ...
  $(document)
    ...
    .on 'submit', '.f-c-catalogue__collection-actions-bar-destroy-form', (e) ->
      $form = $(this)
      $bar = $form.closest('.f-c-catalogue__collection-actions-bar')
      ids = $bar.data('ids')

      if ids
        $form.find('.f-c-catalogue__collection-actions-bar-destroy-input').remove()
        $form.append """
          <input type="hidden" class="f-c-catalogue__collection-actions-bar-destroy-input" name="ids" value="#{ids}">
        """
      else
        e.preventDefault()
