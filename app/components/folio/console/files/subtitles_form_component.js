window.Folio.Stimulus.register('f-c-files-subtitles-form', class extends window.Stimulus.Controller {
  ...
  subtitleDeleted(e) {
    ...
    console.log('Subtitle deleted for language:', language)
  ...
  newSubtitleRemoved(e) {
    const language = e.detail.language
    console.log('New subtitle removed for language:', language)
    this.addedLanguages.delete(language)
    this.updateAddLanguageOptions()
  }

  // Method to refresh language tracking (useful for debugging)
  refreshLanguageTracking() {
    this.addedLanguages.clear()
    this.initializeAddedLanguages()
    this.updateAddLanguageOptions()
  }
    ...
    window.Folio.Api.apiGet(url).then((res) => {
      if (res && res.data) {
        ...
        if (this.hasLanguagesContainerTarget) {
          ...
          console.log('Language added to set:', language)
  ...
  updateAddLanguageOptions() {
    ...
    console.log('Current added languages:', Array.from(this.addedLanguages))
    ...
    console.log('Available options count:', availableOptionsCount)
    ...
    if (addLanguageSection) {
      if (availableOptionsCount === 0) {
        ...
        console.log('Hiding add language section - no options available')
      } else {
        addLanguageSection.style.display = 'block'
        console.log('Showing add language section - options available')
      }
    }
  }
