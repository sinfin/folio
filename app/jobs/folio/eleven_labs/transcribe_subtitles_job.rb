...
class Folio::ElevenLabs::TranscribeSubtitlesJob < Folio::ApplicationJob
  ...
  def perform(video_file)
    ...
    begin
      ...
      Rails.logger.info "[TranscribeSubtitlesJob] Detected language: #{detected_language} (confidence: #{language_probability}), using: #{target_language}"
      ...
      error_message = if e.message.include?("ElevenLabs API error: 400 / Audio is too short")
        "No audio detected in video file."
      else
        e.message
      end
      ...
      # Re-raise unless it's the known "no audio" case
      raise e unless e.message.include?("Audio is too short")
    ensure
