    ...
    class Encoder
      ...
      private
        ...
        def download_s3_to_temp_file(file, temp_file)
          Rails.logger.warn("[CraMediaCloud::Encoder] DEPRECATED: download_s3_to_temp_file used. Use download_to_file_path instead.")
          temp_file.close
          download_to_file_path(file, temp_file.path)
          temp_file.reopen(temp_file.path, "rb")
        end
        ...
        def sftp_client(&block)
          Rails.logger.warn("[CraMediaCloud::Encoder] DEPRECATED: Using legacy sftp_client method. Consider using with_robust_sftp_session instead.")
