...
module Folio::Users::DeviseUserPaths
  ...
  def after_sign_up_path_for(_resource)
    send(Rails.application.config.folio_users_after_sign_up_path)
  end

  def after_accept_path_for(_resource)
    send(Rails.application.config.folio_users_after_accept_path)
  end

  def signed_in_root_path(_resource)
    send(Rails.application.config.folio_users_signed_in_root_path)
  end
end
