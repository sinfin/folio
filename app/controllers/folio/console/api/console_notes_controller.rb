  ...
  def toggle_closed_at
    ...
    if @console_note.valid?
      render json: modify_valid_hash_to_render({
        data: {
          catalogue_tooltip: render(Folio::Console::ConsoleNotes::CatalogueTooltipComponent.new(record: @console_note.target)),
      ...
      if @target.update(params.permit(*console_notes_strong_params))
        render json: modify_valid_hash_to_render({
          data: {
            ...
            catalogue_tooltip: render(Folio::Console::ConsoleNotes::CatalogueTooltipComponent.new(record: @target)),
          }
        })
      else
