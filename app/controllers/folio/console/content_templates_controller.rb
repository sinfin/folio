...
class Folio::Console::ContentTemplatesController < Folio::Console::BaseController
  ...
  def edit
    @content_templates = @klass.by_site(Folio::Current.site)
                               .ordered
  end
  ...
  def update
    @klass.transaction do
      content_template_params.each do |attrs|
        ...
        if destroy == "1"
          if id
            @klass.by_site(Folio::Current.site).find(id).destroy!
          end
        else
          if id
            ct = @klass.by_site(Folio::Current.site).find(id)
            ct.update!(dup)
          else
            @klass.create!(dup.merge(site: Folio::Current.site))
          end
        end
      end
    end
    ...
    def content_template_params
      params.require(:content_template)
            .permit(content_templates_attributes: [
              :content,
              :title,
              *Folio::Current.site.locales.map { |l| "content_#{l}".to_sym },
              :position,
              :_destroy,
              :id
            ])
