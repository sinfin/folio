...
>(({ autosaveIndicatorInfo }, ref) => {
  const [lastSavedAt, setLastSavedAt] = React.useState<Date | null>(
    autosaveIndicatorInfo?.latestRevisionAt
      ? new Date(autosaveIndicatorInfo.latestRevisionAt)
      : null,
  );
  const [hasUnsavedChanges, setHasUnsavedChanges] = React.useState<boolean>(
    autosaveIndicatorInfo?.hasUnsavedChanges ?? false,
  );
  const [failedToAutosave, setFailedToAutosave] =
  ...
  React.useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data?.type === "f-input-tiptap:autosave:auto-saved") {
        ...
        event.data?.type === "f-input-tiptap:autosave:continue-unsaved-changes"
      ) {
        setHasUnsavedChanges(false);
      } else if (
        event.data?.type === "f-input-tiptap:autosave:failed-to-autosave"
      ) {
