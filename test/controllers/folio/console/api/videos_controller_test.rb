...
class Folio::Console::Api::VideosControllerTest < Folio::Console::BaseControllerTest
  ...
  test "update_subtitles legacy method" do
    Folio::File::Video.stub(:transcribe_subtitles_job_class, Folio::OpenAi::TranscribeSubtitlesJob) do
      patch url_for([:update_subtitles, :console, :api, @video, format: :json]), params: {
        file: {
          ...
          "subtitles_en_text" => "00:00:03.000 --> 00:00:04.000\nEnglish subtitle text",
      ...
      # Check that subtitles were created and configured correctly
      cs_subtitle = @video.reload.subtitle_for("cs")
      en_subtitle = @video.subtitle_for("en")

      assert_not_nil cs_subtitle
      assert_not_nil en_subtitle

      assert_equal "00:00:01.000 --> 00:00:02.000\nCzech subtitle text", cs_subtitle.text
      assert cs_subtitle.enabled?, "Czech subtitle should be enabled but was: #{cs_subtitle.enabled}"

      assert_equal "00:00:03.000 --> 00:00:04.000\nEnglish subtitle text", en_subtitle.text
      assert_not en_subtitle.enabled?, "English subtitle should be disabled but was: #{en_subtitle.enabled}"
    end
  end

  test "update_subtitles broadcasts subtitle update" do
    Folio::File::Video.stub(:transcribe_subtitles_job_class, Folio::OpenAi::TranscribeSubtitlesJob) do
      # Mock MessageBus to capture published messages
      published_messages = []
      MessageBus.stub(:publish, ->(channel, message, opts = {}) { 
        ...
        patch url_for([:update_subtitles, :console, :api, @video, format: :json]), params: {
          file: {
            "subtitles_cs_text" => "Czech subtitle text",
            "subtitles_cs_enabled" => "1"
          }
        }
      end
