...
class Dummy::Ui::CardComponent < ApplicationComponent
  THUMB_SIZE = {
    ...
    xs: {
      vertical: { true => "306x208#", false => "370x240#" },
      horizontal: { true => "168x290#", false => "200x354#" },
    }
  }
  ...
  def render_content_box?
    ...
    @render_content_box = render_description? || render_topics? || render_button?
  end
  ...
  def render_description?
    ...
    @render_description = @title.present? || @subtitle.present? || @html.present? || @text.present?
  end

  def render_button?
    return @render_button unless @render_button.nil?
    @render_button = @button_label.present? && @button_variant.present?
  end
end
