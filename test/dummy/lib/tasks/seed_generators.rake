...
class Dummy::SeedGenerator
  ...
  def atoms_controllers(path)
    ...
    replaced_contents = contents.gsub(/\R\s+<%= application_namespace %>::Atom::Blog::Articles::Index:\R\s+-/, "")
  ...
  private
    ...
    def replace_names(str)
      str.gsub("Dummy::", "<%= application_namespace %>::")
         ...
         .gsub(/\bd-atom/, "<%= classname_prefix %>-atom")
         ...
         .gsub(/\bdAtom/, "<%= classname_prefix %>Atom")
