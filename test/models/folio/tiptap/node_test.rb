...
class Folio::Tiptap::NodeTest < ActiveSupport::TestCase
  ...
  test "site validation with valid file" do
    if Rails.application.config.folio_shared_files_between_sites
      ...
      node = Node.new(title: "foo", cover: image)
      node.site = site1

      assert node.valid?
      assert_empty node.errors[:base]

      node = Node.new(title: "bar", cover: image)
      node.site = site2
      assert node.invalid?
    end
  end

  test "site validation with invalid file" do
    if Rails.application.config.folio_shared_files_between_sites
      site1 = create(:folio_site, domain: "site1.localhost", type: "Folio::Site")
      site2 = create(:folio_site, domain: "site2.localhost", type: "Folio::Site")
      image = create(:folio_file_image)
      ...
      assert_includes node.errors[:base], I18n.t("folio.tiptap.errors.file_not_allowed_for_site")
    end
  end

  test "site validation with no site restrictions" do
    if Rails.application.config.folio_shared_files_between_sites
      site1 = create(:folio_site, domain: "site1.localhost", type: "Folio::Site")
      image = create(:folio_file_image)
  ...
  test "site validation without site provided" do
    image = create(:folio_file_image)
  ...
  test "site validation via new_from_params" do
    if Rails.application.config.folio_shared_files_between_sites
      site1 = create(:folio_site, domain: "site1.localhost", type: "Folio::Site")
      site2 = create(:folio_site, domain: "site2.localhost", type: "Folio::Site")
      image = create(:folio_file_image)
      ...
      params = ActionController::Parameters.new({
        type: "Folio::Tiptap::NodeTest::Node",
        data: {
          title: "foo",
          cover_placement_attributes: { file_id: image.id }
        }
      })

      # Valid with correct site
      node1 = Node.new_from_params(params, site: site1)
      assert node1.valid?

      # Invalid with wrong site
      node2 = Node.new_from_params(params, site: site2)
      assert_not node2.valid?
      assert_includes node2.errors[:base], I18n.t("folio.tiptap.errors.file_not_allowed_for_site")
    end
  end
end
