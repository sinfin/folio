...
class Folio::VideoSubtitleTest < ActiveSupport::TestCase
  ...
  test "validates presence of language" do
    ...
    refute @subtitle.valid?
    assert_includes @subtitle.errors[:language], "can't be blank"
  end
  ...
  test "validates uniqueness of language per video" do
    ...
    assert_includes duplicate.errors[:language], "has already been taken"
  end

  test "validates format inclusion" do
    @subtitle.format = 'invalid'
    refute @subtitle.valid?
    assert_includes @subtitle.errors[:format], "is not included in the list"
  end
  ...
  test "validates subtitle format when validate_content is true" do
    ...
    refute @subtitle.valid?
    assert_includes @subtitle.errors[:text], "has invalid subtitle block near line 1"
  end
  ...
  test "mark_transcription_ready! with invalid content keeps subtitle disabled" do
    ...
    freeze_time do
      ...
      refute @subtitle.enabled?
      assert_equal invalid_vtt, @subtitle.text
      assert_equal 'ready', @subtitle.transcription_state
      assert_equal Time.current.iso8601, @subtitle.transcription['completed_at']
      refute @subtitle.validation['is_valid']
  ...
  test "mark_manual_edit! updates edit count without changing transcription state" do
    ...
    freeze_time do
      ...
      assert_equal 1, @subtitle.manual_edits['edit_count']
    end
  end
  ...
  test "processing? returns correct value" do
    refute @subtitle.processing?
    
    @subtitle.update_transcription_metadata('state' => 'processing')
    assert @subtitle.processing?
  end

  test "transcription_ready? returns correct value" do
    refute @subtitle.transcription_ready?
    
    @subtitle.update_transcription_metadata('state' => 'ready')
    assert @subtitle.transcription_ready?
  end

  test "transcription_failed? returns correct value" do
    refute @subtitle.transcription_failed?
    
    @subtitle.update_transcription_metadata('state' => 'failed')
    assert @subtitle.transcription_failed?
  end

  test "status_for_display returns correct status" do
    assert_equal 'pending', @subtitle.status_for_display
    
    @subtitle.update_transcription_metadata('state' => 'processing')
    assert_equal 'processing', @subtitle.status_for_display
    
    @subtitle.update_transcription_metadata('state' => 'failed')
    assert_equal 'failed', @subtitle.status_for_display
    
    @subtitle.update_transcription_metadata('state' => 'ready')
    @subtitle.enabled = true
    assert_equal 'ready', @subtitle.status_for_display
    
    @subtitle.enabled = false
    assert_equal 'ready_but_disabled', @subtitle.status_for_display
    
    @subtitle.update_transcription_metadata('state' => 'manual_override')
    assert_equal 'manual', @subtitle.status_for_display
  end

  test "scopes work correctly" do
    subtitle1 = Folio::VideoSubtitle.create!(video: @video, language: 'cs', enabled: true)
    subtitle2 = Folio::VideoSubtitle.create!(video: @video, language: 'en', enabled: false)
  ...
  test "display_name uses I18n translation" do
    I18n.with_locale(:en) do
      assert_equal "CS", @subtitle.display_name
    end
  end
