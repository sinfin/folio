...
class Folio::Console::Files::Show::Thumbnails::ListGroupComponentTest < Folio::Console::ComponentTest
  def render_group(file:, ratio:, keys:)
  ...
  test "renders representative, ratio label and variants count" do
    ...
    render_group(file:, ratio: "16:9", keys: %w[160x90# 320x180#])

    assert_selector('.f-c-files-show-thumbnails-list-group[data-ratio="16:9"]')
    assert_selector(".f-c-files-show-thumbnails-list-group__label", text: "16×9")
  ...
  test "renders usage label from the thumbnail_ratio_label hook" do
    file = create(:folio_file_image)
    file.update!(thumbnail_sizes: { "160x90#" => { url: "https://example.com/160x90.jpg" } })
    def file.thumbnail_ratio_label(_ratio, _keys)
      "Karta · Infobox"
    end

    render_group(file:, ratio: "16:9", keys: %w[160x90#])

    assert_selector(".f-c-files-show-thumbnails-list-group__usage", text: "Karta · Infobox")
  end
  ...
  test "regular group renders without representative and count" do
    ...
    render_group(file:, ratio: "regular", keys: %w[250x250])

    assert_selector(".f-c-files-show-thumbnails-list-group__label", text: "Verze bez ořezu")
