...
async function handler(event) {
    ...
    request.uri = buildURIFromJWT(payload);

    log("using URL: " + request.uri);

    return request;
}

function buildURIFromJWT(payload) {
    const s3Path = "/" + payload.s3_path;
    const thumbor = payload.thumbor;

    if (thumbor) {
        return "/thumbnail/" + thumbor + s3Path;
    }

    return "/original" + s3Path;
}
...
async function getSecret() {
    ...
    return "a-valid-string-secret-that-is-at-least-256-bits-long";
