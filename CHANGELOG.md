...
- **Console "page is being edited" presence heartbeat**: The heartbeat (`Folio::Console::CurrentUsers::PresencePingComponent`) runs on every console `edit`/`update` even when the editor is alone, re-pings on tab `visibilitychange`, and asks the server for a rendered warning bar (`bar_html`) when another editor appears so the first editor is warned live without a page reload. The warning bar reacts to the heartbeat broadcast (`folio:console:presence-ping`) to remove the plain presence variant once the other user leaves; it no longer pings itself. Closing the tab or navigating away sends a `navigator.sendBeacon` `console_presence_clear` to remove the presence record immediately (`pageshow` from BFCache re-pings to restore it). Revision-based variants (takeover, outdated) are unaffected.
