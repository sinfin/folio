...
### Fixed
- **Console search by email**: Typing a full email address, or its dotted local part, into a console index search finds the record again. The pg_search sanitizer ran the query through `parameterize`, which turned `petr.tobiska@volny.cz` into four ANDed prefix terms while PostgreSQL keeps the address as a single `email` lexeme, so no email search could ever match. The sanitizer now only folds the characters `unaccent` expands into an apostrophe or a backslash (`ŉ` → `'n`), which is what it was added for, and leaves dots and at-signs alone. Affects every `by_query` / `by_label_query` over an email or another dotted value — `Folio::User`, `Folio::Lead`, `Folio::NewsletterSubscription` and host app models.
