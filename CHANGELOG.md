...
### Fixed
- **Local direct uploads**: Return an absolute upload URL and an S3-compatible `ETag` header from the local-file upload endpoint, and exclude that endpoint from Rack Mini Profiler so the required header is preserved in development. Uppy now recognizes and finishes successful uploads instead of reporting misleading S3 CORS or invalid URL errors.
