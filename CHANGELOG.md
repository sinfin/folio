...
- **Direct S3 upload locks**: `Folio::S3::CreateFileJob` now uses a runtime-only uniqueness lock, so a lost queued job cannot leave an enqueue-time lock that blocks `/folio/api/s3/after` retries for the same temp S3 path. `Folio::S3::DeleteJob` keeps its enqueue-to-execution lock.
