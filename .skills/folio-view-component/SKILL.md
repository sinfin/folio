...
- **Templates:** Use `.html.slim` files; keep templates thin; logic belongs in the component class (mostly **`private`** methods). Follow [`.skills/folio-slim/SKILL.md`](../folio-slim/SKILL.md) for formatting conventions (multi-line attributes, multiple `class` attrs, avoiding inline Ruby and `==`).
